...
function getProductNumber {
        product=`cat ${WORKSPACE}/workspace/ept-tool_Release/build.cfg | grep $module | grep $branch | awk -F " " '{print $3}'`
}


function setRstate {

        revision=`cat ${WORKSPACE}/workspace/ept-tool_Release/build.cfg | grep $module | grep $branch | awk -F " " '{print $4}'`
...
function appendRStateToPlatformReleaseXml {
		...
		versionXml="${WORKSPACE}/workspace/ept-tool_Release/src/resources/version/release.xml"
		...
		mv ${WORKSPACE}/workspace/ept-tool_Release/src/resources/version/release.xml ${WORKSPACE}/workspace/ept-tool_Release/src/resources/version/release.${rstate}.xml
